...
// ── PAGE TITLES ──
const pageTitles = {
  home: "GT Apex Marketing — Full Throttle Digital",
  services: "Services — GT Apex Marketing",
  about: "About — GT Apex Marketing",
  contact: "Contact — GT Apex Marketing",
};

// ── PAGE ROUTING ──
const pages = { home, services, about, contact };

window.showPage = function (id, pushState = true) {
  // Render page
  ...
  // Update page title
  document.title = pageTitles[id] || pageTitles.home;
...
window.addEventListener("popstate", (e) => {
  ...
  showPage(id, false); // false = don't push another history entry
});
...
// ── APP BOOT ──
function initApp() {
  // Check URL on first load so direct links like /about work
  const path = window.location.pathname.replace("/", "").toLowerCase();
  const startPage = pages[path] ? path : "home";
